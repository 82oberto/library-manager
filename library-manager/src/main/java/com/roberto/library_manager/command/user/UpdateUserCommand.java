...
public class UpdateUserCommand extends GlobalCommand<Map.Entry<String, User>, UserResponse> {

    private final UserService userService;

    @Override
    protected UserResponse doExecute(Map.Entry<String, User> input) {
        return userService.update(input.getKey(), input.getValue());
    }

    @Override
    protected boolean canExecute(Map.Entry<String, User> input) {
